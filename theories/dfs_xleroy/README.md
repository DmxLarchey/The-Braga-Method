...
let dfs_xl_self x =
  ...
  in dfs_list [x] []
```
Interestingly, the self nested variant `dfs_cycle_self` is the first step towards a variant of  `dfs_book` avoiding the use of the external append/`@` and then `dfs_book` itself. _DLW->JFM: préciser le sens ici..._
```
